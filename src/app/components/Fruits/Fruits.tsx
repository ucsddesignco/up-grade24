...
import Matter, { Constraint, Common, Mouse, MouseConstraint, Vertices } from 'matter-js';
...
export default function Fruits() {
  ...
  useEffect(() => {
    ...
    const leftWall = Bodies.rectangle(0, height - 200, 10, 600, { isStatic: true });
    const rightWall = Bodies.rectangle(width, height - 200, 10, 600, { isStatic: true });
    ...
    if (watermelonSVG.current) {
      ...
      const scaledVertices = Vertices.scale(vertices, watermelonScaling, watermelonScaling, { x: 0, y: 0 });
        ...
        body.bounds.max.y - body.bounds.min.y,
        {
          ...
          render: {
            ...
            sprite: {
              ...
              yScale: 240 / textureHeight * watermelonScaling * .98,
            }
          }
        }
      );
      let scaleTranslate = watermelonScaling * .25 + 1;
    ...
    if (apricotSVG.current) {
      ...
      // Scale the vertices to a new size
      const scaledVertices = Vertices.scale(vertices, apricotScaling, apricotScaling, { x: 0, y: 0 });
      ...
      let scaleTranslateX = apricotScaling * .4;
      let scaleTranslateY = apricotScaling * .4 + 1;
      let constraint = Constraint.create({
        render: {
          visible: false
        },
    ...
    if(cherrySVG.current) {
      ...
      const scaledVertices = Vertices.scale(vertices, cherryScaling, cherryScaling, { x: 0, y: 0 });
      const body = Bodies.fromVertices(
        500,
        200,
        [scaledVertices],
